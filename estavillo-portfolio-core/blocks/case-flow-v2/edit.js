/**
 * estavillo/case-flow-v2 — editor.
 *
 * Mismo dato y misma UI que estavillo/case-flow: lo que cambia es cómo se
 * DIBUJA la bifurcación, no cómo se edita. Dos convenciones que conviene
 * tener presentes al armar el grafo en "Va hacia":
 *
 *   · Si una salida apunta a un nodo que NO está en la línea principal, ese
 *     nodo es una pantalla intermedia: el trazo sale del vértice SUPERIOR del
 *     rombo y la pantalla vuelve a bajar al mismo destino que la salida
 *     principal.
 *   · Si una salida apunta a un nodo ANTERIOR, es un bucle: sale del vértice
 *     INFERIOR y vuelve por debajo, sin tarjeta intermedia. No hace falta
 *     crear un nodo "volver a X" — alcanza con apuntar a X.
 *
...
(function (wp, ui) {
	...
	wp.blocks.registerBlockType('estavillo/case-flow-v2', {
		edit: function (props) {
			...
			return el(
				...
				el(
					...
					el(
						...
						el(SelectControl, {
							label: __('Nodes per row (desktop)', 'estavillo-portfolio-core'),
							help: __(
								'Decides which nodes share a row. For a branch to read as a fork, the decision and the step its branch rejoins have to land in the SAME row.',
								'estavillo-portfolio-core'
							),
							value: String(a.columns || 4),
							options: [
								{ label: '3', value: '3' },
								{ label: '4', value: '4' },
								{ label: '5', value: '5' },
